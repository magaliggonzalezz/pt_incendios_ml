...
LOCAL_RELATIVE = Path("data_deploy/capas_web/inegi/relieve_mde")
PREFIX = "capas_web/inegi/relieve_mde"
...
def encontrar_raiz(inicio: Path) -> Path:
    ...
    raise FileNotFoundError(
        "No se encontró data_deploy/capas_web/inegi/relieve_mde/manifest.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Verifica y sube a R2 los tiles del relieve MDE INEGI."
    )
    ...
    if not archivos:
        raise RuntimeError("No hay archivos de relieve para subir")
    ...
    if args.solo_verificar and pendientes:
        ...
        print("RELIEVE MDE VERIFICADO EN R2")
